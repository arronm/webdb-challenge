...
1. Explain the difference between `RDBMS` and `SQL`.
  - RDBMS is the software itself that manages storing, updating, deleting that is based on a relational model
  - SQL is the query language to interact with the RDMBS
1. Why do tables need a `primary key`?
  - Need? It is not required, but it is HIGHLY recommended and considered a bad practice to not have one since it's free--especially if you need to join tables. It provides logical consistency, data integrity and has performance benefits.
1. What is the name given to a table column that references the primary key on another table.
  - FK! Foreign Key
1. What do we need in order to have a _many to many_ relationship between two tables.
  - Smoke and mirrors! There is no many to many! (We need a connecting table).
